...
export default {
    command: 'سكلوود',
    aliases: ['scsearch', 'soundcloud', 'scloud'],
    category: 'موسيقى',
    description: 'بحث فور تراككس ون سووندكلوود',
    usage: '.سكلوود <أغنية نامي>',
    async handler(sock, message, args, context) {
        ...
        try {
            ...
            if (tracks.length === 0) {
                return await sock.sendMessage(chatId, {
                    text: "âŒ *No tracks found!*\nOnly found user profiles. Try searching for specific songs."
                }, { quoted: message });
            }
            const limit = Math.min(5, tracks.length);
            let resultText = `ðŸŽµ *SoundCloud Results*\n`;
            resultText += `ðŸ“Š Found ${totalFound} results (${tracks.length} tracks)\n\n`;
            for (let i = 0; i < limit; i++) {
                ...
                resultText += `ðŸ‘¤ Artist: ${track.user_id ? 'Available' : 'Unknown'}\n`;
                resultText += `â±ï¸ Duration: ${minutes}:${seconds.toString().padStart(2, '0')}\n`;
                resultText += `ðŸ‘‚ Plays: ${track.playback_count?.toLocaleString() || 'N/A'}\n`;
                resultText += `â¤ï¸ Likes: ${track.likes_count?.toLocaleString() || 'N/A'}\n`;
                resultText += `ðŸ’¬ Comments: ${track.comment_count?.toLocaleString() || 'N/A'}\n`;
                resultText += `ðŸŽ¼ Genre: ${track.genre || 'Unknown'}\n`;
                resultText += `ðŸ”— Link: ${track.permalink_url}\n\n`;
            }
