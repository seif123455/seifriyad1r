...
export default {
    command: 'سيبيا',
    aliases: ['vintage', 'sepia'],
    category: 'أدوات',
    description: 'تحويل ان صورة تو سيبيا',
    usage: 'رد تو ان صورة ويته .سيبيا',
    async handler(sock, message, args, context) {
        const chatId = context.chatId || message.key.remoteJid;
        try {
            const quoted = message.message?.extendedTextMessage?.contextInfo?.quotedMessage;
            if (!quoted?.imageMessage) {
                return await sock.sendMessage(chatId, { text: 'ðŸ§¡ *Sepia Image*\n\nReply to an image to convert it to sepia\n\nUsage:\n.sepia' }, { quoted: message });
            }
